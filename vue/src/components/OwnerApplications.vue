<template>
  ...
  >
    Applicant Name: {{ applications.name }} Application Number:
    {{ applications.applicantId }} Property Number:
    {{ applications.propertyId }} Application Satus: {{ applications.status }}
    <span> Date Applied: {{ applications.date }} </span>
    <span v-if="viewDetails">
      Applcant BirthDate: {{ applications.birthDate }} Applcant Email:
      {{ applications.email }} Applcant PhoneNumber:
      {{ applications.phone }} Applcant Salary:
      {{ applications.salary }} Applcant Social:
      {{ applications.social }} Applcant Current Address:
      {{ applications.address }} Applcant BirthDate:
      {{ applications.birthDate }}
      <button
        class="btn btn-primary"
        v-on:click="viewDetails = false"
        type="button"
      >
        Show Less
      </button>
